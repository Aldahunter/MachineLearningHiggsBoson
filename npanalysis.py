...
from sklearn import metrics.roc_auc_score, metrics.confusion_matrix

import nptreeclassifiers as TC
...
def test_classifier(observables, classifier):
    return classifier.predict(observables)
...
def find_p_threshold(test_results):
...
def get_scores(test_results, p_threshold=0.5):
    scores = {}
    tn, fp, fn, tp = confusion_matrix(test_results, p_threshold)
...
def SN_classifier(classifier, data_df, classifier_kwargs, rseed=None):
    ...
    observables, labels = {}, {}
    observables['train'], labels['train'] = TC.df_to_ML_input(data_df['train'])
    observables['test'],  labels['test']  = TC.df_to_ML_input(data_df['test'])

    classifier.train(observables['train'], labels['train'], **classifier_kwargs)
    
    test_results = test_classifier(observables['test'], classifier)
    train_results = test_classifier(observables['train'] classifier)
...
def analyse_classifier(classifier, sgn_df, bkg_df, input_observables, classifier_kwargs):
    ...
    observables, labels = {}, {}
    observables['train'], labels['train'] = TC.df_to_ML_input(data['train'])
    observables['test'],  labels['test']  = TC.df_to_ML_input(data['test'])
